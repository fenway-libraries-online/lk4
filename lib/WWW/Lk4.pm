...
sub new {
    ...
    my %file2data;
    my $self = bless {
        'config_file' => '/usr/local/lk4/conf/lk4.conf',
        'config_dir'  => '/usr/local/lk4/conf/lk4.d',
        'data_dir'    => '/usr/local/lk4/data',
...
sub handle_cgi {
    my ($self, $q) = @_;
    my %env = $self->request_environment($q);
    my $req = $env{'$path_info'} . $env{'$query_string'};
    my %result = $self->resolve($req, %env);
    my ($ok, $status, $uri, $menu) = @result{qw(ok status uri menu)};
    if (defined $uri) {
        $self->print_redirect($uri, $status);
    }
    elsif (defined $menu) {
...
sub print_menu {
    ...
    my $content = $self->process_template($fh, {
        'menu' => $menu,
    });
